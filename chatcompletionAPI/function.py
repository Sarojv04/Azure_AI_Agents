...
def get_weather(location):
   ...
   #fetching latitude and longitude of the specific location respectively
    url = "http://api.openweathermap.org/geo/1.0/direct?q=" + location + "&limit=1&appid=" + os.getenv("OPENWEATHER_API_KEY")
    response=requests.get(url)
    get_response=response.json()
    latitude=get_response[0]['lat']
    longitude = get_response[0]['lon']

    url_final = "https://api.openweathermap.org/data/2.5/weather?lat=" + str(latitude) + "&lon=" + str(longitude) + "&appid=" + os.getenv("OPENWEATHER_API_KEY")
...
def get_user_info(user_id: int) -> str:
    ...
    """
    mock_users = {
        1: {"name": "Ram", "email": "ram@example.com"},
        2: {"name": "sita", "email": "sita@example.com"},
        3: {"name": "gita", "email": "gita@example.com"},
    }
    user_info = mock_users.get(user_id, {"error": "User not found in the database."})
